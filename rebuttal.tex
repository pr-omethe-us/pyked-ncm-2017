...
here. The manuscript has been significantly revised.
...
\begin{response}
    %
    We thank the reviewer for their comments. Detailed responses to each comment are
    below.

    Regarding the manuscript, please see our response to your next point; in addition, we added more
    detail to our examples in response to Reviewer \#2's comments, which increased the length of
    the manuscript slightly.
    %
\end{response}
...
\begin{response}
    In response to one of your later comments, we have extended our examples to make them
    more usable as-is, and also added figures to show results from these examples on
    \textbf{pages 16 and 19}.
\end{response}
...
\begin{response}
    ...
    the ChemKED schema do not lend themselves to easy informal\slash working use. We presently recommend that
    users who wish to use ChemKED files in this way include an incomplete reference field. We have
    modified the ChemKED schema (and \textbf{page 5} of the manuscript) in response to this comment
    so that the only required subfields of the reference field are the authors and the year.
...
\begin{response}
    ...
    Moreover, all existing databases have a similar requirement (whether explicitly stated or
...
\begin{response}
    ...
    text on \textbf{page 3} to v0.3.0, which we released in conjunction with submitting
...
\begin{response}
    %
    We have truncated the datapoints list on \textbf{pages 12 and 17} as the reviewer suggests.
    %
\end{response}
...
\begin{response}
    %
    We have modified the example on \textbf{pages 17--19} to include more
...
\begin{response}
    %
    We have added a paragraph to the conclusion on \textbf{page 20}.
    %
\end{response}
