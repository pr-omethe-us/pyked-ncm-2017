...
\pk{} relies on pytest~\autocite{pytest:3.0.1} for unit testing, and currently has
100\% line and branch coverage for all of the executable code in the repository. This
means that the tests execute every line of code at least once, and every branch statement
(e.g., if-then-else statement) is tested so that every condition is executed at least
once.
...
ensures that the test suite is run after every change to the source code, the
documentation is up-to-date with the latest changes, and releasing new versions is as
...
\url{https://github.com/pr-omethe-us/PyKED}~\autocite{PyKED}. Installation instructions
are available in the online documentation at
\url{https://pr-omethe-us.github.io/PyKED/install.html} and can be done via binary
packages or from the source code.
