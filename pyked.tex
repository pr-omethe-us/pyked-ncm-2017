...
open availability.
\todo[inline, author=Bryan]{This reads more as a footnote, maybe consider putting it there instead?}
...
\todo[author=Bryan,inline]{It would maybe be good to give a bit of Frenklach's perspective here, rather than just throwing in the citation}

While the community has not widely adopted a standard format or the practice of openly \todo{this second bit feels like a slightly separate point, and it makes the second clause here a bit awkward}
...
PrIMe~\autocite{Frenklach:2007bm,You:2011hy} represented an admirable \todo{Maybe its just me, but this feels a bit sarcastic} first attempt at standardization, but a number of limitations
...
\todo{Also because the standard was closed?}
...
less ambitious in scope\todo{its not clear to me how this is less ambitious than the other projects}: the \ck{} project will enable easy sharing and use of fundamental combustion data, for the (primary) purposes of
...
\todo[inline, author=Bryan]{We should probably also mention the CloudFlame project from KAUST.}
...
\pybox{ChemKED} class. The \pybox{ChemKED} class constructor takes the file
...
The \pybox{ChemKED} class has one instance method defined, called
...
be accomplished from a package repository, using either \pybox{pip} to install
...
\pybox{tests} directory of the PyKED repository,
...
The \ck{} file used in this example can be found online.\todo{add url}{} The
...
Recognizing that the community has a need for an open and standardized data
serialization format, in this paper we have presented the \ck{} data format for
describing measurements taken from fundamental combustion experiments. \ck{}
files are formatted using the YAML language and are plain-text, human- and
machine-readable, and easy to construct.

We have also presented a Python-based tool, \pk{}, for validating and working
...
measurements from rapid compression machines and shock tubes, including facility
specific effects from each type of experiment.
