...
Finally, we create the processor \pybox|Pool| (with 4 processes) and send the jobs out to run:
%
\begin{pythonbox}
...
Finally, we presented several examples of the use of \pk{} to interpret and interact with data
stored in \ck{} files. Both examples are also available online as Jupyter Notebook files in the
GitHub repository and in the Supplementary Material. The first example involved the simulation of a
rapid compression machine experiment, including the facility-specific effects that are critical to
accurately predict ignition delay experiments. The second example demonstrated the use of built-in
Python libraries to automatically run simulations of several shock tube experiments in parallel,
