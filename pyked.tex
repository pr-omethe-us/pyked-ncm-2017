...
keyword. Cantera and PyKED both must be imported into the session so that we can work with the code.
In the case of Cantera, we will use many functions from the library, so we assign it an abbreviation
(\pybox|ct|) for convenience. From PyKED, we will only be using the \pybox|ChemKED| class, so this
...
for mass, energy, and species conservation.
...
autoignition problems. Although we do not show the details here, a reference implementation of a
